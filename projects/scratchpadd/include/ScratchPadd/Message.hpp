...
#define MessageTypes                                                           \
  ScratchPadd::MessageType::Point, ScratchPadd::MessageType::Triangle,         \
      ScratchPadd::MessageType::Text, ScratchPadd::MessageType::Control
...
struct Double : public BaseControlType {
  ...
  Double(double initialValue = 0.0,
         ...
         std::optional<std::function<void(double)>> controlHandler =
             std::nullopt) {
    value = initialValue;
...
struct Integer : public BaseControlType {
  ...
  Integer(int32_t initialValue = 0.0,
          ...
          std::optional<std::function<void(int32_t)>> controlHandler =
              std::nullopt) {
    value = initialValue;
    enforcableRange = range;
    handler = controlHandler;
...
struct String : public BaseControlType {
  ...
  String(std::string initialValue,
         ...
         std::optional<std::function<void(std::string)>> controlHandler =
             std::nullopt) {
    value = initialValue;
    enforcableRange = range;
    handler = controlHandler;
